...
class DynaMaze:
    ...
    def predict_lead_to_S(self, S, A, PQueue):
        # up
        S_left = S - np.array([1,0])
        S_up = S - np.array([0,1])
        S_right = S - np.array([-1,0])
        S_down = S - np.array([0,-1]) 
        S_bar = [S_left, S_up, S_right, S_down]
    ...
    def run_priortized_sweeping(self):
        count = 0
        update_count = 0
        PQueue = set()
        while not (self.current_location == self.end_location).all():
            ...
            _, max_a = self.greedy(self.current_location, self.Q)
            # Model(S,A) <- R,S'
            self.model[(tuple(S),A)]=(R,tuple(S_new)) # (d)
            P = abs(R + self.gamma * self.fetch_Q(S_new, max_a) - self.fetch_Q(S, A)) # (e)
            if P > self.theta:
                PQueue.add((P, S[0], S[1], A))
            if len(PQueue) > 0:
                for i in range(0, self.n):
                    ...
                    self.predict_lead_to_S(S, A, PQueue) # whole loop
                    update_count+=1
        self.current_location = self.start_location.copy()
        return update_count
    ...
    def iterate(self, iteration):
        ...
        accumulated_timestep = [0]
        for i in range(0,iteration):
            timestep = self.run_priortized_sweeping()
            sum_timestep += timestep
            accumulated_reward.append(i)
            accumulated_timestep.append(sum_timestep)
        self.draw_plot( accumulated_reward, accumulated_timestep, self.n, tag)
